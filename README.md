# 👋 Olá, eu sou Douglas Karan

🎓 **Bacharel em Física | Suporte Técnico | Automação & Sistemas**
📍 Fortaleza, Ceará, Brasil

---

## 🚀 Sobre mim

Sou profissional de **Suporte Técnico** com formação em **Física pela UECE** e experiência em aplicações da **Microsoft Power Platform** (Power Automate, SharePoint e Power BI). Atuo com análise de falhas, automação de processos, orientação a usuários e foco total na **experiência do cliente**.

Atualmente, estou aprofundando meus estudos em **Administração de Sistemas**, **automação com n8n** e **Typebot**, buscando evoluir para áreas de **infraestrutura, servidores Linux e ambientes corporativos**.

---

## 🛠️ Tecnologias & Competências

* ⚙️ **Automação**: Power Automate, n8n, Typebot
* 🖥️ **Sistemas**: GNU/Linux, Servidores Linux, Máquinas Virtuais
* 📊 **Dados**: Power BI (análise e visualização)
* 🌐 **Plataformas**: SharePoint, Z-Pro Multi Atendimento, CRM Perfex
* 🤖 **RPA**: Automação Robótica de Processos
* 🧑‍💻 **Suporte Técnico** e Atendimento ao Usuário

---

## 💼 Experiência Profissional

### 🏢 Tecno Center — *Suporte Técnico*

📅 **Nov/2023 – Atual**
📍 Fortaleza, CE

* Suporte a aplicações baseadas na **Microsoft Power Platform**
* Atendimento e resolução de problemas no **Z-Pro Multi Atendimento**
* Análise de falhas, orientação a usuários e melhoria contínua de processos

---

### 🏥 Hapvida — *Agente de Call Center*

📅 **Abr/2023 – Nov/2023**

* Atendimento ao cliente
* Marcação de consultas e exames
* Comunicação clara e foco em resolução rápida

---

### 📞 Contax S.A. — *Operador de Telemarketing Ativo e Receptivo*

📅 **Abr/2022 – Jul/2022**

* Serviço de Atendimento ao Consumidor (SAC)
* Suporte e orientação ao cliente

---

### 🧪 Universidade Estadual do Ceará — *Monitor de Laboratório de Física*

📅 **Mar/2019 – Abr/2021**

* Apoio em experimentos didáticos
* Auxílio a alunos e professores em atividades laboratoriais

---

### ✈️ Aeroclube do Ceará — *Auxiliar Financeiro e de Operações*

📅 **Fev/2016 – Jan/2017**

* Atendimento ao cliente e gestão financeira
* Operações de solo, documentação e suporte operacional

---

## 🎓 Formação Acadêmica

* 🎓 **Bacharelado em Física** — UECE *(2016 – 2022)*
* 💻 **Técnico em Informática para Internet** — IFCE *(2025 – 2026)*

---

## 📜 Certificações & Cursos

* Introdução à Análise de Dados — **Microsoft Power BI**
* Introdução às Máquinas Virtuais do **Microsoft Azure**
* **SharePoint**
* Atendimento ao Público
* Curso de Agente de Operações de Pátio

---

## 🌍 Idiomas

* 🇬🇧 **Inglês** — Intermediário (Limited Working)

---

## 🏆 Destaques

* Monitor do Laboratório de Física 2
* Forte experiência com atendimento ao cliente e suporte técnico

---

## 📫 Contato

* 📧 Email: **[kkaran.dk@gmail.com](mailto:kkaran.dk@gmail.com)**
* 💼 LinkedIn: [Douglas Karan](https://www.linkedin.com/in/douglas-karan-araújo-medeiros-2082321a5)

---

⭐ *Sinta-se à vontade para explorar meus repositórios e acompanhar minha evolução em automação, Linux e infraestrutura!*
